...
public class NewDailyCommand extends ListenerAdapter {

    private static final Random RANDOM = new Random();

    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (!event.getName().equals("newdaily")) return;
        ...
        if (IslandStorage.hasIsland(userId)) {
            event.reply("❌ Sul on juba saar! Kasuta **/islanddaily** oma päevase auhinna saamiseks.")
                    .setEphemeral(true).queue();
            return;
        }
        ...
        long newBalance = IslandStorage.getCoins(userId);
    ...
    private MessageEmbed buildSuccessEmbed(String username, long reward, long newBalance) {
        return new EmbedBuilder()
                ...
                .addField("Saare Mündid", "🏝️ " + newBalance + " münti", true)
                .setFooter("Tule tagasi 24 tunni pärast! • Osta saar /buyisland käsuga!")
                .build();
    }
